...
INSTANCE_TFVARS="instance.auto.tfvars"
...
# ---------------------------------------------------------------------------
log "Recording instance identity"
# instance_name/environment/gcp_project_id/gcp_region were previously only
# ever passed as -var flags below -- nowhere on disk, so a fresh session (or
# a different machine) had no way to run a plain `terraform plan` without
# reverse-engineering these from live resource labels. auto.tfvars is
# Terraform's own auto-loaded convention (no -var-file flag needed, same as
# terraform.tfvars), and unlike that file this one holds no secrets -- just
# identifiers already visible in this project's own resource labels -- so
# it's deliberately exempted from .gitignore's blanket *.tfvars rule.
# Rewritten every run so a changed REGION argument stays current; the other
# three can't actually change across runs (they select the Terraform
# workspace / state bucket / project itself).
cat > "$INSTANCE_TFVARS" <<EOF
# Auto-generated by scripts/bootstrap.sh -- identifies which instance this
# checkout's terraform.tfvars applies to. Safe to commit: no secrets, just
# this deployment's name/tier/project/region.
instance_name  = "${INSTANCE_NAME}"
environment    = "${ENVIRONMENT}"
gcp_project_id = "${PROJECT_ID}"
gcp_region     = "${REGION}"
EOF
echo "  wrote ${INSTANCE_TFVARS}"
