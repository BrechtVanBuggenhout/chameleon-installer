...
# time), this also redeploys each one with the image it just pushed via a
# direct `gcloud run deploy` -- faster than round-tripping through a full
# `terraform apply`. Also writes build_own_images.auto.tfvars recording
# those exact same image URIs, so a later `terraform apply` (image IS
# Terraform-managed now, unlike before) sees the same value this script
# just deployed and doesn't revert it -- these three services' `:latest`
# tag never changes format between runs, only Terraform's own record of it
# needs to stay in sync. First-time installs (no matching service exists
# yet) still get the same auto.tfvars written, so a subsequent
# `bootstrap.sh` picks these images up with no manual tfvars editing.
#
# Usage:
...
# Writes the three image URIs to build_own_images.auto.tfvars itself --
# no manual terraform.tfvars editing needed before running bootstrap.sh.
set -euo pipefail
...
# ---------------------------------------------------------------------------
log "Recording image references for Terraform"
# image is Terraform-managed now (see key_vault.tf/console.tf/
# pii_ingestor_worker.tf) -- without this, the next `terraform apply` would
# see these variables' committed/default values disagree with what
# gcloud run deploy just did above and revert it. auto.tfvars is
# Terraform's own auto-loaded convention (same pattern as bootstrap.sh's
# instance.auto.tfvars) -- no secrets, safe to commit.
cat > build_own_images.auto.tfvars <<EOF
# Auto-generated by scripts/build-own-images.sh -- keeps Terraform in sync
# with the images this script just built, tagged, and deployed. Safe to
# commit: no secrets, just this project's own Artifact Registry URIs.
...
EOF
echo "  wrote build_own_images.auto.tfvars"
...
First install? build_own_images.auto.tfvars (just written) already points
bootstrap.sh at these images -- no manual terraform.tfvars editing needed.
